...
function onLoad(saved_data)
    self.max_typed_number = 9 -- requirement for onNumberTyped()
    if saved_data ~= "" then
...
--BW allows typing a number while hovering the container to spawn that many dice
function onNumberTyped(player_color, number_typed)
    click_roll(_, player_color, _, number_typed)
    return true --BW prevents default behaviour (draw from bag)
end
...
function click_roll(_, color, _, clicks)
    clicks = clicks or 1
    ...
    for _ = 1,clicks do
        if rollInProgress==nil and denyRoll==false and anyRollInProgress(color)==false then
            ...
            spawnedDie.interactable = false --BW allows clicking through the die
...
function rollTimerUpdate(param)
    --Timer starting --BW replaced Timer.destroy and Timer.create with Wait.stop and Wait.time
    if timerID then Wait.stop(timerID) end
    timerID = Wait.time(function()
        if timerID then timerID = nil end
        timer_rollDice(param)
    end, setting.rollDelay)
end
